...
def count_repeats(xs, x):
    ...
    # binary search function that finds the highest index with a value >= x
    ...
    # binary search function that finds the lowest index with a value < x
    ...
    step_1 = greater(0, len(xs) - 1)
    step_2 = less(0, len(xs) - 1)
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    But the actual minimization code will all be a variant of
    this binary search.

    WARNING:
    The doctests below are not intended to pass on your code,
    and are only given so that you have an example of what the output
    should look like.
    ...
    m2_val = f(m2)
    hi_val = f(hi)
    if lo_val >= m1_val >= m2_val:
        return argmin(f, m1, hi, epsilon)
    elif hi_val >= m2_val >= m1_val:
...
def find_boundaries(f):
    '''
    Returns a
    https://www.when2meet.com/?11194683-eBSB7
    https://www.when2meet.com/?11194683-eBSB7 tuple(lo,hi).
    If f is a convex function, then the minimum is guaranteed to be
    between lo and hi.
